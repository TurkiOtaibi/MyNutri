...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "(superseded_by_id IS NULL AND superseded_at IS NULL) OR (superseded_by_id IS NOT NULL AND superseded_at IS NOT NULL)",
    ...
    op.create_index(
        ...
        ["principal_id"],
        unique=True,
        postgresql_where=sa.text("superseded_at IS NULL AND status='selected'"),
    )
    op.create_index(
    ...
    op.create_index(
        "ix_priority_eval_launch",
        "weekly_priority_evaluation",
        ["evaluation_mode", "evaluation_diary_date"],
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "evidence_kind = 'analysis_fact'", name="ck_weekly_priority_evidence_kind"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "ix_weekly_priority_evidence_principal_date",
        "weekly_priority_evidence_ref",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "sequence_number >= 1 AND version >= 1", name="ck_behavior_goal_versions"
        ),
        ...
        sa.CheckConstraint(
            "(state <> 'deferred' OR (deferred_at IS NOT NULL AND deferred_until IS NOT NULL)) AND (state <> 'paused' OR paused_at IS NOT NULL) AND (state <> 'completed' OR completed_at IS NOT NULL) AND (state <> 'incomplete' OR reviewed_at IS NOT NULL) AND (state <> 'rejected' OR rejected_at IS NOT NULL) AND (state <> 'ended' OR ended_at IS NOT NULL)",
        ...
        sa.CheckConstraint(
            "(last_progress_attempt_analysis_id IS NULL AND last_progress_attempt_analysis_revision_id IS NULL AND last_progress_attempt_analysis_revision IS NULL) OR "
    ...
    op.create_table(
        ...
        sa.Column("id", _uuid(), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("goal_id", _uuid(), nullable=False),
        sa.Column("principal_id", _uuid(), nullable=False),
        sa.Column("root_goal_id", _uuid(), nullable=False),
        sa.Column("previous_goal_id", _uuid()),
        ...
        sa.CheckConstraint(
            "actor_type IN ('owner','system')", name="ck_behavior_goal_history_actor"
        ),
        sa.CheckConstraint(
            "event_type IN ('offered','accept','edit','defer','reject','change','changed','pause','resume','end','completed','evidence_reopened','progress_updated','historical_evidence_changed','finalized_completed','finalized_incomplete','repeated_from_previous_window')",
            name="ck_behavior_goal_history_event",
        ),
        sa.CheckConstraint(
            "reason IS NULL OR reason IN ('not_relevant','too_difficult','prefer_other','pause_tracking','other','owner_requested','evidence_superseded')",
            name="ck_behavior_goal_history_reason",
        ),
    )
    op.create_index(
        "ix_behavior_goal_history_goal_time",
        "behavior_goal_history",
    ...
    op.create_index(
        ...
        "behavior_goal_history",
        ["principal_id", sa.text("occurred_at DESC"), sa.text("id DESC")],
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "response_status BETWEEN 200 AND 599", name="ck_behavior_goal_command_status"
        ),
    )
    op.create_index(
        "ix_behavior_goal_command_completed",
        "behavior_goal_command_idempotency",
        ["principal_id", sa.text("completed_at DESC")],
    )
    ...
    op.create_table(
        ...
        sa.Column("eligibility_diary_date", sa.Date(), nullable=False),
        sa.Column("deferred_until", sa.DateTime(timezone=True)),
        sa.Column("status", sa.String(24), nullable=False),
        sa.Column("provider_receipt_digest", sa.String(64)),
        ...
        sa.CheckConstraint(
            "reminder_type IN ('midweek','endweek_review')", name="ck_behavior_goal_reminder_type"
        ),
        sa.CheckConstraint(
            "channel IN ('in_app','external')", name="ck_behavior_goal_reminder_channel"
        ),
        sa.CheckConstraint(
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_behavior_goal_command_completed", table_name="behavior_goal_command_idempotency"
    )
    ...
    op.drop_index(
        "ix_behavior_goal_history_principal_occurred_id", table_name="behavior_goal_history"
    )
    ...
    op.drop_index(
        "ix_weekly_priority_evidence_principal_date", table_name="weekly_priority_evidence_ref"
    )

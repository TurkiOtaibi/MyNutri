...
def _seed_diary(session: Session, principal_id: UUID, count: int) -> list[DiaryEntry]:
    ...
    food_id = uuid5(NAMESPACE_URL, f"plan025-food-{principal_id}")
    ...
    if food is None:
        food = Food(
            ...
            name="Plan025 Food",
            normalized_name=f"plan025 food {principal_id}",
...
def test_plan025_postgresql_budgets_read_only_and_bounded_pages(
    ...
    first_plan = _explain(
        session,
        "SELECT id, entry_date, meal_type, quantity, food_id, recorded_unit_amount, "
...
def test_plan025_migration_rehearsal_catalog_and_reversibility(
    ...
    assert heads.stdout.strip() == "f47a2c9d6e13 (head)"

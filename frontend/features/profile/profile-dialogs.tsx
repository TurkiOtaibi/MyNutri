...
export function ProfileConfirm({ title, description, safeLabel, confirmLabel, pending = false, onClose, onConfirm, restoreFocusRef }: { title: string; description: string; safeLabel: string; confirmLabel: string; pending?: boolean; onClose: () => void; onConfirm: () => void; restoreFocusRef?: RefObject<boolean> }) {
  return <ProfileSheet title={title} onClose={onClose} restoreFocusRef={restoreFocusRef} dismissible={!pending} busy={pending}><div className="profile-confirm"><p>{description}</p><button className="btn primary" type="button" onClick={onClose} disabled={pending}>{safeLabel}</button><button className="btn" type="button" onClick={onConfirm} disabled={pending}>{pending ? <><LoaderCircle className="spin" size={17} /> جارٍ الحفظ…</> : confirmLabel}</button></div></ProfileSheet>;
}

...
test.describe("@diary @page-refinement compact Diary page", () => {
  ...
  test("@p0 another-year date includes year, Today returns home, and day arrows cross a Sunday boundary", async ({ page }) => {
    ...
    await expect(page.locator('.compact-week-day[aria-current="date"]')).toHaveAttribute("aria-label", /السبت/);
  ...
  test("@p0 compact Food row omits brand/macros, supports row edit, options, and delete failure recovery", async ({ page, foodsApi }) => {
    ...
    await expect(row.locator('button button, [role="button"] button, button [role="button"]')).toHaveCount(0);
    await page.getByRole("button", { name: `تعديل ${food.name}` }).click();
    await expect(page.getByRole("dialog", { name: "تعديل الكمية والقسم" })).toBeVisible();
    await page.keyboard.press("Escape");
    const menuTrigger = page.getByRole("button", { name: `خيارات ${food.name}` });
    await menuTrigger.click();
    const editAction = page.getByRole("menuitem", { name: "تعديل" });
    const deleteAction = page.getByRole("menuitem", { name: "حذف" });
    await expect(editAction).toBeFocused();
    await editAction.press("ArrowDown");
    await expect(deleteAction).toBeFocused();
    await deleteAction.press("ArrowUp");
    await expect(editAction).toBeFocused();
    await editAction.press("Escape");
    await expect(menuTrigger).toBeFocused();
    await expect(editAction).toHaveCount(0);
    await menuTrigger.click();
